...
gage_data <- read.table(theURL, skip = 29, header=TRUE, 
                        sep='\t', stringsAsFactors = T) 

#Update the column headers
colnames(gage_data) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", 
                        "89193_00065_cd")
#Tidy the data
gage_data <- gage_data %>% 
...
gage_data <- read.table(theURL, skip = 30, header=TRUE, 
                        sep='\t', stringsAsFactors = T) 
...
the_capacity <- the_url %>% 
  html_nodes("td:nth-child(3)") %>% 
  html_text()

the_capacity

net_generation <- the_url %>% 
  html_nodes("td:nth-child(4)") %>% 
  html_text()

net_generation

total_retail <- the_url %>% 
  html_nodes("td:nth-child(5)") %>% 
  html_text()
