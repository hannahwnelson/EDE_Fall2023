...
```{r DataExercise-1a}
f_high <- c(81, 83, 84, 84, 78, 72, 80, 82, 86, 85)
f_low<- c(55, 56, 58, 61, 62, 64, 64, 65, 65, 63)
```
...
c_high <- round(c_high, digits=0)
c_high

c_low <- (f_low-32)*(5/9)
c_low <- round(f_low, digits=0)
c_low
...
```{r DataExercise-1c}
forecast <- data.frame("high temp in F"= f_high, "low temp in F" = f_low, "high temp in C" = c_high, "low temp in C" = c_low)
forecast
```
...
```{r DataExercise-1d}
summary(forecast)
sd(forecast$high.temp.in.F)
sd(forecast$low.temp.in.F)
sd(forecast$high.temp.in.C)
sd(forecast$low.temp.in.C)
```
