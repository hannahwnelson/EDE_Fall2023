...
sites <- read.csv(here('Data/Raw/NWIS_SiteFlowData_NE_Raw.csv')) #here() finds the desired file within the project folder
here() #home directory (project folder)
dim(sites) #look at dimensions of data (152x4)
...
levels(sites$dow) <- c(levels(sites$dow),"1","2","6","7") #adding sun, mon, fri, & sat to existing levels
...
ggplot(sites,aes(x=dow)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE) #drop=FALSE so that levels with no values are not dropped
...
```{r}
sites$date <- factor(sites$date)

sites$date <- format(date, "%m")

levels(sites$date)

sites$date <- factor(sites$date, levels=c("01","02","03","04","05","06","07","08","09","10","11","12"))

levels(sites$date)

ggplot(sites,aes(x=date)) +
  geom_bar() + 
  scale_x_discrete(drop=FALSE)
