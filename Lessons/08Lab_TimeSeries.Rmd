...
vis_miss(Rainfall_BR_processed)
vis_miss(Temp_BR_processed)
...
rain.ts <- ts(data = BR_complete$Rainfall, frequency = 12, start = c(1901,1))


temp.ts <- ts(data = BR_complete$Temperature_C, frequency = 12, start = c(1901,1))
...
temp.ts.decomp <- stl(temp.ts, s.window = "periodic")
plot(temp.ts.decomp)

rain.ts.decomp <- stl(rain.ts, s.window = "periodic")
plot(rain.ts.decomp)
